...
import com.github.malayP.decorations.register.AllTileEntity.beltHubType
...
class BeltHub : HorizonBlockWithTileEntity(Properties.create(Material.IRON)) {
    override fun createTileEntity(state: BlockState?, world: IBlockReader?): TileEntity = BeltHubTileEntity()
}

class BeltHubTileEntity : TileEntity(beltHubType.get()) {

}

class BeltHubTileEntityRender(dispatcher: TileEntityRendererDispatcher) :
    TileEntityRenderer<BeltHubTileEntity>(dispatcher) {
    companion object {
        val model = BeltHubModel()
        val texture = modResourcesLocation("textures/tile_entity/transmission/belt_hub/belt_hub.png")
    }

    override fun render(
        tileEntityIn: BeltHubTileEntity,
        partialTicks: Float,
...
class BeltHubModel : Model(RenderType::getEntitySolid) {
